...
class BinanceTestnetRateSource(RateSourceBase):
    def __init__(self):
        super().__init__()
        self._binance_testnet_exchange: Optional[BinanceExchange] = None  # delayed because of circular reference
    ...
    async def get_prices(self, quote_token: Optional[str] = None) -> Dict[str, Decimal]:
        ...
        results = {}
        tasks = [
            self._get_binance_prices(exchange=self._binance_testnet_exchange),
        ]
        task_results = await safe_gather(*tasks, return_exceptions=True)
        for task_result in task_results:
            if isinstance(task_result, Exception):
                self.logger().error(
                    msg="Unexpected error while retrieving rates from Binance Testnet. Check the log file for more info.",
    ...
    def _ensure_exchanges(self):
        if self._binance_testnet_exchange is None:
            self._binance_testnet_exchange = self._build_binance_connector_without_private_keys(domain="testnet")
