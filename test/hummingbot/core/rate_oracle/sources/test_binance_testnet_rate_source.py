...
from hummingbot.core.rate_oracle.sources.binance_testnet_rate_source import BinanceTestnetRateSource


class BinanceTestnetRateSourceTest(IsolatedAsyncioWrapperTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def setup_binance_responses(self, mock_api, expected_rate: Decimal):
        pairs_url = web_utils.public_rest_url(path_url=CONSTANTS.EXCHANGE_INFO_PATH_URL, domain='testnet')
        ...
        binance_prices_global_url = web_utils.public_rest_url(path_url=CONSTANTS.TICKER_BOOK_PATH_URL, domain='testnet')
    ...
    @aioresponses()
    async def test_get_binance_testnet_prices(self, mock_api):
        expected_rate = Decimal("10")
        self.setup_binance_responses(mock_api=mock_api, expected_rate=expected_rate)

        rate_source = BinanceTestnetRateSource()
